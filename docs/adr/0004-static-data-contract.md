...
- `/build-info.json`: app version, deterministic fallback commit label, repository URL, PayPal URL.
...
The frontend can show version without a backend. When GitHub's public API is reachable, the app shows the latest `main` commit for the repository; otherwise it falls back to `/build-info.json`.
